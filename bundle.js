...
class Election {
  ...
  make_circle(t, r) {
    ...
    return `<svg width="80" height="80">
		...
		<circle cx="50%" cy="50%" r="` + r / t * 40 + `" fill="#e6007a" />
                <text x="50%" y="50%" text-anchor="middle" fill="#fff" dy=".3em">` + r.toFixed(2) + `</text>
	      </svg>`;
  }

  make_vote_circle(t, r) {
    console.log([t, r]);
    return `<svg width="80" height="80">
		<circle cx="50%" cy="50%" r="40" fill="#000" />
		<circle cx="50%" cy="50%" r="` + r / t * 40 + `" fill="#e6007a" />
                <text x="50%" y="50%" text-anchor="middle" fill="#fff" dy=".3em">` + r.toFixed(0) + ` votes</text>
	      </svg>`;
  }
  ...
  new_candidate_with_name(name) {
    ...
    let el = $('<tr>').attr('id', this.id + name).append($('<td>').html(name)).append($('<td>').attr('id', this.id + name + '_approvalshare')).append($('<td>').attr('id', this.id + name + '_voteshare')).append($('<td>').html('=')).append($('<td>').attr('id', this.id + name + '_supportshare'));

    if (this.style > 0) {
      el.append($('<td>').attr('id', this.id + name + '_score').attr('class', 'small-td'));
    }

    el.append($('<td>').attr('id', this.id + name + '_elected'));
  ...
  update() {
    ...
    for (let c of this.state.candidates) {
      ...
      for (let s of c.score_record) {
        if (s[2] == "elected") {
          sr += "<span style='color:#e6007a; font-size:2em;'>" + s[0] + " (" + s[1].toFixed(3) + ")</span><br>";
        } else {
          sr += s[0] + " (" + s[1].toFixed(3) + ")<br>";
        }
      }

      if (c.elected) {
        $('#' + this.id + c.name + "_elected").html(c.elected_round);
      } else {
        $('#' + this.id + c.name + "_elected").html("not elected");
      }
      ...
      if (this.style > 1) {
        $('#' + this.id + c.name + '_supportshare').html(this.make_circle(this.state.total_budget, c.support));
        $('#' + this.id + c.name + '_approvalshare').html(this.make_circle(this.state.total_budget, c.approval));
      } else {
        $('#' + this.id + c.name + '_supportshare').html(this.make_circle(this.state.total_budget, c.support));
        $('#' + this.id + c.name + '_approvalshare').html(this.make_circle(this.state.total_budget, c.approval));
      }

      $('#' + this.id + c.name + '_voteshare').html(this.make_vote_circle(this.state.total_votes, c.num_votes));
    }
    ...
    if (this.style > 0) {
      sort.sortTable(this.id + "candidates", 6);
    } else {
      sort.sortTable(this.id + "candidates", 5);
    }
...
const seq_phragmen = (assignment, num_to_elect) => {
  ...
  for (let round = 0; round < num_to_elect; round++) {
    ...
    for (let candidate of assignment.candidates) {
      if (!candidate.elected) {
        candidate.score_record.push([round + 1, candidate.score, "not elected"]);
      }
    }
    ...
    if (elected_candidate) {
      ...
      elected_candidate.score_record[elected_candidate.score_record.length - 1][2] = "elected";
  ...
  for (let candidate of assignment.candidates) {
    candidate.budget_proportion = candidate.support / assignment.total_budget;
    candidate.budget_proportion_no_conviction = candidate.support / assignment.total_budget;
    candidate.votes_proportion = candidate.num_votes / assignment.total_votes;
  }
